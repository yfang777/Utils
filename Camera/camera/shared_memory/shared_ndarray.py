...
class SharedNDArray(Generic[SharedT]):
    ...
    lock
        (Optional) multiprocess.Lock to manage access to the SharedNDArray. This is only created if
    ...
    dtype: np.dtype
    lock: Optional[multiprocess.synchronize.Lock]
    ...
    def __init__(
        ...
        ----------
        shm
            `multiprocess.shared_memory.SharedMemory` object or name for connecting to an existing block
        ...
        lock : bool, optional
            If True, create a multiprocess.Lock object accessible with the `.lock` attribute, by default
            False.  If passing the `SharedNDArray` as an argument to a `multiprocess.Pool` function this
            should not be used -- see this comment to a Stack Overflow question about `multiprocess.Lock`:
    ...
    def create_from_array(
        ...
        ----------
        mem_mgr
            Running `multiprocess.managers.SharedMemoryManager` instance from which to create the
