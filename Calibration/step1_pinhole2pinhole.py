...
# Board geometry
# ----------------------------
def make_board_cells_sim(nx=6, ny=5, square_len=0.05):
...
def make_board_cells_real(nx=6, ny=5, square_len=0.05):
    """Return list of (4,3) quads in world coords (meters)."""
    quads = []
    for ix in range(nx):
        for iy in range(ny):
...
def main():
    ...
    parser.add_argument("--square_len", type=float, default=0.05)
    parser.add_argument("--H", type=int, default=480)
    parser.add_argument("--W", type=int, default=640)
    parser.add_argument("--real_params", type=str, default="../camera_calibration/calib_results/cam_params.pkl")
    parser.add_argument("--real_serial", type=str, default="239222300412")
    parser.add_argument("--xml", type=str, default="../camera_calibration/assets/scene_new_robot.xml")
    parser.add_argument("--camera_name", type=str, default="camera_239222300412")
    parser.add_argument("--real_rgb", type=str, default="", help="Optional path to a real RGB image (BGR is fine).")
    parser.add_argument("--save_prefix", type=str, default="step1")
    ...
    Ex_rs = params[args.real_serial]["extrinsic"]  # 4x4 world in camera coordinates”
    K_rs  = params[args.real_serial]["intrinsic"]  # 3x3
    ...
    with mujoco.Renderer(model, H, W) as renderer:  # (model, width, height)
        ...
        rgb_mj = renderer.render()  # BGR uint8
        Ex_mj, K_mj = get_mj_cam(data, args.camera_name, W, H)
    ...
    # 1) Visualize SIM first and save
    quads3d = make_board_cells_sim(args.nx, args.ny, args.square_len)
    img_sim = render_overlay(H, W, quads3d, Ex_mj, K_mj, base=None, nx=args.nx, ny=args.ny)
    save_image(f"{args.save_prefix}/sim.png", img_sim)

    # 2) Visualize REAL and save
    quads3d = make_board_cells_real(args.nx, args.ny, args.square_len)
    img_real = render_overlay(H, W, quads3d, Ex_rs, K_mj, base=None, nx=args.nx, ny=args.ny)
    save_image(f"{args.save_prefix}/real.png", img_real)

    # 3) Make combined overlays and save
    # 3a) Side-by-side concat
    concat = np.hstack([img_sim, img_real])
    save_image(f"{args.save_prefix}/concat.png", concat)

    # 3b) Alpha-blended overlay (SIM over REAL); if no real base, blend with gray
    base_for_blend = img_real
    blend = cv2.addWeighted(img_sim, args.blend_alpha, base_for_blend, 1.0 - args.blend_alpha, 0.0)
